# OlConnect

Современный клиент VPN и прокси-соединений для Android 8+. Маршрутизирует трафик через системный `VpnService`, обеспечивая обход блокировок и цензуры с использованием инновационных транспортов маскировки и стандартных протоколов.

---

## Поддерживаемые протоколы

### 1. OpenFlux
- Высокоскоростной L3 IP-туннель, инкапсулированный в протоколы облачных редакторов документов:
  - **Yandex Docs** (поддерживаются как новый движок Volga, так и классический редактор);
  - **Mail.ru Docs** (редактор документов Mail.ru).
- **Сквозное шифрование AES-256-GCM**: аутентифицированное шифрование всего IP-трафика с защитой от повторного воспроизведения (replay attack) и деривацией ключей через scrypt. Ключ задается через параметр `&k=<key>` в URI или редактируется в настройках профиля.
- **RFC 792 ICMP Port Unreachable Fast TCP Fallback**: немедленный возврат ICMP-ошибки «порт недоступен» на заблокированные UDP-порты, что заставляет приложения (браузеры, мессенджеры) мгновенно переключаться на TCP без ожидания таймаутов.
- Встроенный резолвер DNS-over-HTTPS (DoH: Cloudflare, Google, Quad9, Yandex) и выбор пользовательского DNS.
- Автоматическая компрессия пакетов (LZ4 `compressedTransport` и batched streaming).
- Работает напрямую без необходимости создания комнат видеоконференций.

### 2. OlConnect (WebRTC)
- Инкапсуляция TCP/IP-трафика внутри сессий видеоконференций доверенных белых сервисов:
  - **Яндекс.Телемост** (`vp8channel`, `videochannel`);
  - **Jitsi Meet** (`vp8channel`, `datachannel`, `seichannel`, `videochannel`);
  - **WB Stream** (`vp8channel`, `datachannel`).
- Поддержка аппаратного и программного кодирования VP8 с контролем целостности пакетов.

### 3. Стандартные протоколы (на базе ядра Xray)
- **VLESS**: поддержка транспортов TCP/raw, WebSocket, gRPC, XHTTP, а также протоколов безопасности TLS, REALITY и VLESS Vision flow.
- **VMess** и **Trojan**.
- **Shadowsocks**.

---

## Импорт конфигураций и подписки

- **Глубокие ссылки (Deep Links)**: клик по ссылкам вида `olconnect://`, `openflux://`, `vless://`, `vmess://`, `trojan://`, `ss://`.
- **Спецификация ссылки OpenFlux**:
  ```
  openflux://<yandex|mailru>?url=<url_документа>[&t=<транспорт>][&k=<ключ_шифрования>][&d=<dns>]#<название>
  ```
- **QR-коды**: сканирование камерой или импорт изображения QR из галереи.
- **Вставка из буфера обмена** или импорт файлов конфигураций.
- **Умные подписки с зеркалами**:
  - Поддержка ссылок на подписки `https://.../sub/<slug>` сервера OlConnect Manager.
  - Автоматический fallback на зашифрованное зеркало в **Яндекс.Диске** (AES-256-GCM), если основной сервер недоступен или заблокирован.

---

## Маршрутизация

- **Пресеты маршрутизации**:
  - *Весь трафик через VPN*;
  - *Россия напрямую (Bypass Russia)*: трафик к российским ресурсам (`geoip:ru`, `geosite:category-ru`) идет напрямую, зарубежный трафик — через защищенный туннель.
- **Выборочная маршрутизация приложений (Per-App)**:
  - Все приложения;
  - Все, кроме выбранных;
  - Только выбранные приложения.
- **Пользовательские правила маршрутизации**:
  - Точные домены, суффиксы доменов (`.domain.com`), IP-адреса, CIDR-диапазоны и пакеты приложений (`package:com.example.app`);
  - Действия для правил: «Через VPN», «Напрямую (Direct)», «Блокировать (Block)».
- **Защита от утечки реального IP сервера (SO_BINDTODEVICE bypass protection)**:
  - Изоляция сетевого интерфейса `tun0` от вредоносных приложений и системных сканеров (таких как `RKNHardering`, `curl --interface tun0`);
  - Пакетная валидация владельца соединения (UID) через `ConnectivityManager.getConnectionOwnerUid` на Android 10+ с кешированием состояний и мгновенным сбросом неразрешенных сессий (TCP RST / ICMP Port Unreachable).

---

## Встроенное обновление приложения

Приложение умеет автоматически проверять обновления и безопасно скачивать APK-файлы релизов с GitHub прямо через активный туннель или напрямую, с проверкой контрольной суммы SHA-256 и сертификата подписи.

---

## Сборка

Сборка нативных компонентов и приложения осуществляется через GitHub Actions. Локальные требования для сборки:
- JDK 17
- Android SDK 36 (NDK 28.2)
- Go (согласно `native/mobilecore/go.mod`)

---

## Лицензия

Proprietary / Open Source dependencies (см. `THIRD_PARTY_NOTICES.md`).

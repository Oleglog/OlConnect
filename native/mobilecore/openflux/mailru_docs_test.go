...
func TestNormalizeMailruWeblink(t *testing.T) {
	tests := []struct {
		input    string
		expected string
	}{
	...
	for _, tc := range tests {
		actual := normalizeMailruWeblink(tc.input)
		if actual != tc.expected {
			t.Errorf("normalizeMailruWeblink(%q) = %q, expected %q", tc.input, actual, tc.expected)
		}
	}
}
